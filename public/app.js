...
async function showSyncExistingDnsModal() {
  ...
  try {
    ...
    const summaryHtml = `<div class="dns-sync-summary dns-sync-summary-detailed">
      ...
      <div><strong>${Number(result.unmatchedCount || 0)}</strong><span>未匹配域名</span></div>
      ...
      <div><strong>${Number(result.scannedDomains || 0)}</strong><span>系统域名记录</span></div>
    </div>`;
    ...
    if (!records.length) {
      box.innerHTML = `${warningHtml}${summaryHtml}${zoneHtml}<div class="empty">未发现可同步的 DNS 记录。请查看上方扫描数据和下方跳过原因。</div>${skippedHtml}<div class="modal-actions"><button class="btn secondary" data-rescan-sync type="button">重新扫描</button><button class="btn primary" data-close-sync type="button">关闭</button></div>`;
