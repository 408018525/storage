...
async function showSyncExistingDnsModal() {
  openModal('同步已有 DNS', '读取 Cloudflare 中已存在、但尚未同步到系统的 DNS 记录', `
  ...
  try {
    ...
    const warningHtml = (result.warnings || []).length
      ? `<div class="notice warning dns-sync-warnings"><strong>部分根域名读取失败：</strong><br>${(result.warnings || []).map(esc).join('<br>')}</div>`
      : '';
    if (!records.length) {
      box.innerHTML = `${warningHtml}<div class="empty">未发现需要同步的 DNS 记录。</div><div class="modal-actions"><button class="btn primary" data-close-sync type="button">关闭</button></div>`;
    ...
    box.innerHTML = `
      ${warningHtml}
      <div class="dns-sync-summary">
        <div><strong>${records.length}</strong><span>条待同步记录</span></div>
        <div><strong>${Number(result.scannedDomains || 0)}</strong><span>个已注册域名</span></div>
        <div><strong>${Number(result.scannedZones || 0)}</strong><span>个 Cloudflare Zone</span></div>
      </div>
      ...
      <div class="table-wrap dns-sync-table-wrap">
        <table class="dns-sync-table">
          <thead><tr><th></th><th>所属域名</th><th>主机记录</th><th>类型</th><th>记录内容</th><th>TTL</th><th>代理</th></tr></thead>
          <tbody>${records.map((record, index) => `<tr>
            <td><input type="checkbox" data-existing-dns-index="${index}" aria-label="选择 DNS 记录"></td>
            <td><strong>${esc(record.domain)}</strong><br><small>${esc(record.username || '—')}</small></td>
      ...
      <div class="modal-actions">
        <button type="button" class="btn secondary" data-cancel-sync>取消</button>
        <button type="button" class="btn primary" id="import-existing-dns" disabled>同步所选 DNS</button>
      </div>`;
    applyI18n(box);
    ...
    const updateSelection = () => {
      ...
      count.textContent = lang() === 'en' ? `${selected.length} selected` : `已选择 ${selected.length} 条`;
    ...
    box.querySelector('[data-cancel-sync]')?.addEventListener('click', closeModal);
    importButton.addEventListener('click', async () => {
      ...
      try {
        const imported = await api('/api/admin/dns/import-existing', {
          method:'POST',
          body:{ records:selected.map(record => ({ applicationId:record.applicationId, cfRecordId:record.cfRecordId })) },
        });
        closeModal();
        toast(lang() === 'en'
          ? `Imported ${imported.imported || 0} DNS records; skipped ${imported.skipped || 0}.`
          : `已同步 ${imported.imported || 0} 条 DNS 记录，跳过 ${imported.skipped || 0} 条。`, 'success');
    ...
    box.innerHTML = `<div class="notice danger">${esc(error.message)}</div><div class="modal-actions"><button class="btn primary" data-close-sync type="button">关闭</button></div>`;
    box.querySelector('[data-close-sync]')?.addEventListener('click', closeModal);
    applyI18n(box);
  }
}
...
async function renderAdminApplications() {
  ...
  try {
    ...
    shell('域名审核', `<section class="card"><div class="section-head"><div><h2>域名审核</h2><p>先审核域名；审核通过后，用户才能进入域名管理添加 DNS 解析。</p></div><button class="btn primary" id="sync-existing-dns" type="button">同步已有 DNS</button></div><div class="table-wrap"><table><thead><tr><th>域名</th><th>用户</th><th>DNS</th><th>状态</th><th>到期</th><th>操作</th></tr></thead><tbody>${rows || '<tr><td colspan="6">暂无申请</td></tr>'}</tbody></table></div></section>`);
...
function positionAnalyticsTooltip(tip,event,target){
  const vw=document.documentElement.clientWidth,vh=document.documentElement.clientHeight,margin=12,gap=18;
  ...
  let left=px+gap,top=py-tr.height/2;
...
function bindAnalyticsChartInteractions(root=document) {
  ...
  root.querySelectorAll('.interactive-chart').forEach(box=>{
    ...
    box.querySelectorAll('[data-chart-tip],[data-chart-tip-html]').forEach(el=>{
      ...
      el.addEventListener('pointerenter',show);el.addEventListener('pointermove',show);el.addEventListener('focus',show);el.addEventListener('blur',hide);
    });
