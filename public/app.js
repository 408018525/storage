...
function renderPublicHome() {
  ...
  const suffixCards = suffixes.slice(0,6).map((item,index) => `<article class="public-suffix-card"><div><span>0${index+1}</span><h3>*.${esc(item.suffix)}</h3></div><p>${item.label?esc(item.label):pub('当前开放申请','Currently open for applications')}</p><small>${pub('可直接查询是否可注册','Check availability instantly')}</small><a href="#/available">${pub('立即查询','Check now')} →</a></article>`).join('');
  app.innerHTML = publicShell('home', `
    ${notice}
    <section class="public-hero">
      <div class="public-hero-badge">FREE SUBDOMAIN · DNS CONTROL</div>
      <h1>${pub('你的域名，','Your domain, ')}<em>${pub('即刻上线','ready to launch')}</em></h1>
      <p>${esc(site.subtitle || pub('快速注册并管理您的专属免费域名','Register and manage your free subdomain'))}${pub('。从查询、申请、审核到 DNS 解析，在一个清晰的工作流中完成。','. Check, apply, review, and configure DNS in one clear workflow.')}</p>
      ${publicDomainSearchHtml('home-domain-search')}
      <div class="public-hero-actions"><a class="btn primary" href="${state.me?'#/apply':'#/register'}">${pub('开始使用','Get Started')}</a><a class="btn secondary" href="#/available">${pub('查询域名','Check Domain')}</a></div>
    </section>
    <section class="public-stats" data-public-stats>
      <div><strong data-stat="users">—</strong><span>${pub('活跃用户','Active Users')}</span></div>
      <div><strong data-stat="domains">—</strong><span>${pub('正常域名','Active Domains')}</span></div>
      <div><strong data-stat="dns">—</strong><span>${pub('DNS 记录','DNS Records')}</span></div>
      <div><strong data-stat="suffixes">—</strong><span>${pub('开放根域名','Open Roots')}</span></div>
    </section>
    <section class="public-section"><header><span>01</span><div><h2>${pub('为什么使用本站','Why use this service')}</h2><p>${pub('把免费二级域名申请和日常 DNS 管理放在同一个简单界面里。','Keep free subdomain applications and daily DNS management in one simple interface.')}</p></div></header>${publicFeatureCards()}</section>
    <section class="public-section public-soft-section"><header><span>02</span><div><h2>${pub('当前开放域名','Currently Open Domains')}</h2><p>${pub('这里展示管理员当前开放申请的根域名；是否可注册请进入“可用域名”实时查询。','These are the root domains currently open for applications. Use Available Domains for a real-time check.')}</p></div><a href="#/featured">${pub('查看全部','View all')} →</a></header><div class="public-suffix-grid">${suffixCards || `<div class="public-empty">${pub('当前暂无开放根域名。','No root domains are currently open.')}</div>`}</div></section>
    <section class="public-section"><header><span>03</span><div><h2>${pub('从注册到上线','From registration to launch')}</h2><p>${pub('流程简单，同时保留必要的审核与安全控制。','A simple workflow with necessary review and security controls.')}</p></div></header><div class="public-steps"><article><b>1</b><h3>${pub('创建账户','Create Account')}</h3><p>${pub('完成注册和验证。','Complete registration and verification.')}</p></article><article><b>2</b><h3>${pub('查询域名','Check Domain')}</h3><p>${pub('先确认目标二级域名可以注册。','Confirm the desired subdomain is available.')}</p></article><article><b>3</b><h3>${pub('提交申请','Submit Application')}</h3><p>${pub('选择根域名并提交审核。','Choose a root domain and submit for review.')}</p></article><article><b>4</b><h3>${pub('配置 DNS','Configure DNS')}</h3><p>${pub('审核通过后添加解析记录。','Add DNS records after approval.')}</p></article><article><b>5</b><h3>${pub('项目上线','Go Live')}</h3><p>${pub('等待 DNS 生效即可使用。','Wait for DNS propagation and go live.')}</p></article></div></section>
    <section class="public-section public-infra"><header><span>04</span><div><h2>${pub('技术与基础设施','Technology & Infrastructure')}</h2><p>${pub('本站域名管理系统运行于 Cloudflare Workers，并使用 Cloudflare DNS API 管理授权记录。','The service runs on Cloudflare Workers and manages authorized DNS records through the Cloudflare DNS API.')}</p></div></header><div class="public-infra-grid"><article><b>☁</b><div><h3>Cloudflare</h3><p>Workers · DNS · D1 · KV</p></div></article><article><b>◫</b><div><h3>${pub('响应式界面','Responsive UI')}</h3><p>${pub('桌面、平板和手机统一访问。','Desktop, tablet, and mobile.')}</p></div></article><article><b>⌘</b><div><h3>${pub('系统化管理','Unified Management')}</h3><p>${pub('审核、日志、消息、DNS 在同一控制台。','Reviews, logs, messages, and DNS in one dashboard.')}</p></div></article></div></section>
    <section class="public-section"><header><span>05</span><div><h2>${pub('常见问题','FAQ')}</h2><p>${pub('开始前最常见的几个问题。','Common questions before getting started.')}</p></div><a href="#/faq">${pub('查看全部','View all')} →</a></header>${publicFaqHtml(5)}</section>
    <section class="public-cta"><div><span>${pub('准备好了吗？','Ready?')}</span><h2>${pub('申请一个属于你的免费二级域名','Claim your free subdomain')}</h2><p>${pub('先查询可用性，然后登录或注册进入域名管理中心。','Check availability first, then log in or register to continue.')}</p></div><div><a class="btn primary" href="${state.me?'#/apply':'#/register'}">${pub('开始使用','Get Started')}</a><a class="btn secondary" href="#/knowledge">${pub('阅读知识库','Read Knowledge Base')}</a></div></section>
  `, 'public-home');
  bindPublicDomainSearch('home-domain-search');
  loadPublicStats();
}
...
function renderPublicNavigation() {
  ...
  app.innerHTML = publicShell('navigation', `
    <section class="public-nav-reference-head"><h1>${pub('导航','Navigation')}</h1><p>${pub('本站全部公开页面一览。','A directory of the site’s public pages.')}</p></section>
    <section class="public-nav-directory reference-style">${groups.map(([title,links]) => `<article><h2>${esc(title)}</h2><div class="public-nav-reference-links">${links.map(([name,href,desc]) => `<a href="${href}"><b>${esc(name)}</b><span>${esc(desc)}</span></a>`).join('')}</div></article>`).join('')}</section>
  `, 'public-navigation');
}
...
async function route() {
  ...
  if (hash === '#/home') return renderPublicHome();
