...
function publicFooter() {
  ...
  return `<footer class="public-footer">
    <div class="public-footer-grid">
      ...
      <div><b>${pub('信息','Information')}</b><a href="#/about">${pub('关于本站','About')}</a><a href="#/contact">${pub('联系我们','Contact')}</a><a href="#/faq">${pub('常见问题','FAQ')}</a><a href="#/terms">${pub('服务协议','Terms')}</a><a href="#/privacy">${pub('隐私政策','Privacy')}</a></div>
...
function renderPublicHome() {
  ...
  const heroSearch = site.publicHomepageShowSearch === false ? '' : `<div class="public-home-v113-tool"><div class="public-home-v113-tool-head"><span>${pub('实时查询','LIVE CHECK')}</span><b>${pub('先确认，再申请','Check first, apply second')}</b></div>${publicDomainSearchHtml('home-domain-search', true)}<p>${pub('查询只返回当前可用状态，不公开域名归属或账户信息。','The check returns availability only and does not expose ownership or account information.')}</p></div>`;
  const stats = site.publicHomepageShowStats === false ? '' : `<section class="public-stats public-home-v113-stats" data-public-stats><div><strong data-stat="users">—</strong><span>${pub('活跃用户','Active Users')}</span></div><div><strong data-stat="domains">—</strong><span>${pub('正常域名','Active Domains')}</span></div><div><strong data-stat="dns">—</strong><span>${pub('DNS 记录','DNS Records')}</span></div><div><strong data-stat="suffixes">—</strong><span>${pub('开放根域名','Open Roots')}</span></div></section>`;
  const features = site.publicHomepageShowFeatures === false ? '' : `<section class="public-section public-home-v113-section"><header><span>01</span><div><h2>${pub('一个入口，完成域名日常管理','One place for daily domain management')}</h2><p>${pub('首页负责查询与了解服务，登录后进入控制台处理申请、审核状态与 DNS。','Use the public site for discovery and the dashboard for applications, review status, and DNS.')}</p></div></header>${publicFeatureCards()}</section>`;
  const domains = site.publicHomepageShowDomains === false ? '' : `<section class="public-section public-soft-section public-home-v113-section"><header><span>02</span><div><h2>${pub('现在可以申请的后缀','Root domains open now')}</h2><p>${pub('这里只展示开放入口，不用公开用户域名或账户数据。','Only open root domains are shown here; user domains and account data stay private.')}</p></div><a href="#/featured">${pub('查看全部','View all')} →</a></header><div class="public-suffix-grid">${suffixCards || `<div class="public-empty">${pub('当前暂无开放根域名。','No root domains are currently open.')}</div>`}</div></section>`;
  const process = site.publicHomepageShowProcess === false ? '' : `<section class="public-section public-home-v113-section"><header><span>03</span><div><h2>${pub('操作路径一眼看懂','A workflow you can understand at a glance')}</h2><p>${pub('查询、申请、审核、解析各自独立，减少误操作。','Availability, application, review, and DNS are separate steps to reduce mistakes.')}</p></div></header><div class="public-steps"><article><b>1</b><h3>${pub('查询','Check')}</h3><p>${pub('先确认名称当前可用。','Confirm the name is currently available.')}</p></article><article><b>2</b><h3>${pub('申请','Apply')}</h3><p>${pub('登录后选择根域名并提交。','Log in, select a root domain, and submit.')}</p></article><article><b>3</b><h3>${pub('审核','Review')}</h3><p>${pub('按当前规则完成审核。','Review follows the current site policy.')}</p></article><article><b>4</b><h3>${pub('解析','DNS')}</h3><p>${pub('通过后配置授权的 DNS 类型。','Configure allowed DNS types after approval.')}</p></article><article><b>5</b><h3>${pub('维护','Maintain')}</h3><p>${pub('后续在控制台管理状态与记录。','Manage status and records from the dashboard.')}</p></article></div></section>`;
  const infra = site.publicHomepageShowInfrastructure === false ? '' : `<section class="public-section public-infra public-home-v113-section"><header><span>04</span><div><h2>${pub('系统怎么工作','How the system works')}</h2><p>${pub('公开页面、业务控制台和 Cloudflare DNS 分工明确，避免把内部配置暴露到前台。','Public pages, the dashboard, and Cloudflare DNS have separate responsibilities, keeping internal configuration private.')}</p></div></header><div class="public-infra-grid"><article><b>⌕</b><div><h3>${pub('公开查询层','Public lookup')}</h3><p>${pub('只提供可用性和公开说明。','Availability and public documentation only.')}</p></div></article><article><b>⌘</b><div><h3>${pub('业务控制台','Service dashboard')}</h3><p>${pub('账户、申请、审核、DNS 与日志。','Accounts, applications, review, DNS, and logs.')}</p></div></article><article><b>☁</b><div><h3>Cloudflare</h3><p>Workers · D1 · KV · DNS API</p></div></article></div></section>`;
  const faq = site.publicHomepageShowFaq === false ? '' : `<section class="public-section public-home-v113-section"><header><span>05</span><div><h2>${pub('第一次使用？先看这些','New here? Start with these answers')}</h2><p>${pub('把最容易遇到的问题留在首页，详细内容放到独立知识库。','Keep common questions on the homepage and detailed guidance in the standalone Knowledge Base.')}</p></div><a href="#/faq">${pub('查看全部','View all')} →</a></header>${publicFaqHtml(4)}</section>`;
  app.innerHTML = publicShell('home', `
    ${notice}
    <section class="public-home-v113-hero layout-${attr(layout)}">
      <div class="public-home-v113-copy">
        <div class="public-home-v113-badge">${esc(badge)}</div>
        <h1>${esc(title)} <em>${esc(highlight)}</em></h1>
        <p>${esc(description)}</p>
        <div class="public-home-v113-actions"><a class="btn primary" href="${state.me?'#/apply':'#/register'}">${esc(primaryText)}</a><a class="btn secondary" href="#/available">${esc(secondaryText)}</a></div>
      </div>
      ${heroSearch}
    </section>
    ${stats}${features}${domains}${process}${infra}${faq}
    <section class="public-cta public-home-v113-cta"><div><span>${pub('下一步','NEXT')}</span><h2>${pub('从查询一个名称开始','Start by checking one name')}</h2><p>${pub('不需要登录即可先确认可用性；需要申请时再进入账户流程。','Check availability without logging in, then enter the account flow when you are ready to apply.')}</p></div><div><a class="btn primary" href="#/available">${pub('查询域名','Check Domain')}</a><a class="btn secondary" href="#/knowledge">${pub('阅读知识库','Read Knowledge Base')}</a></div></section>
  `, `public-home public-home-v113 layout-${layout}`);
...
function publicNavigationGroups() {
  ...
  return [
    ...
    [pub('约定','Policies'), [
      ...
      [pub('服务条款','Terms of Service'), '#/terms', pub('使用本站服务需要遵守的条款','Rules for using the service')],
      [pub('隐私政策','Privacy Policy'), '#/privacy', pub('数据收集、使用与保护说明','How data is collected, used, and protected')],
      [pub('联系我们','Contact'), '#/contact', pub('支持与管理员联系入口','Support and admin contact options')],
      [pub('举报滥用','Report Abuse'), '#/contact', pub('举报违法、钓鱼、仿冒或垃圾内容','Report illegal, phishing, impersonation, or spam content')],
    ]],
  ];
}
...
function renderPublicNavigation() {
  ...
  app.innerHTML = publicShell('navigation', `
    <section class="public-nav-hub-v113-head"><div><span>FLORE DIRECTORY</span><h1>${pub('站点导航','Site Directory')}</h1><p>${pub('按使用场景找到入口，而不是把所有链接挤在同一排。','Find the right entry by task instead of packing every link into one row.')}</p></div><a class="btn secondary" href="#/home">${pub('返回首页','Back Home')}</a></section>
    <section class="public-nav-hub-v113">${groups.map(([title,links],groupIndex) => `<article><div class="public-nav-hub-v113-title"><span>${String(groupIndex+1).padStart(2,'0')}</span><h2>${esc(title)}</h2></div><div>${links.map(([name,href,desc],linkIndex) => `<a href="${href}"><i>${String(linkIndex+1).padStart(2,'0')}</i><span><b>${esc(name)}</b><small>${esc(desc)}</small></span><em>↗</em></a>`).join('')}</div></article>`).join('')}</section>
  `, 'public-navigation public-navigation-v113');
}
...
function renderPublicContact() {
  app.innerHTML = publicShell('', `
    <section class="public-page-hero"><span>CONTACT</span><h1>${pub('联系我们','Contact')}</h1><p>${pub('遇到注册、审核、DNS、账户或滥用问题时，可以按下面的方式获取帮助。','Use the options below for registration, review, DNS, account, or abuse issues.')}</p></section>
    <section class="public-contact-grid"><article><i>?</i><h2>${pub('先查知识库','Check the Knowledge Base')}</h2><p>${pub('大多数 DNS、域名状态与申请问题都可以在公开知识库中找到说明。','Most DNS, domain-status, and application questions are covered in the public Knowledge Base.')}</p><a href="#/knowledge">${pub('打开知识库','Open Knowledge Base')} →</a></article><article><i>✉</i><h2>${pub('登录后联系管理员','Contact Admin After Login')}</h2><p>${pub('具体账户与域名问题建议登录后通过帮助中心提交，便于管理员定位对应记录。','For account-specific issues, log in and submit through Help Center so admins can identify the relevant records.')}</p><a href="${state.me?'#/help':'#/login'}">${state.me?pub('进入帮助中心','Open Help Center'):pub('登录后提交','Login to Submit')} →</a></article><article><i>!</i><h2>${pub('举报滥用','Report Abuse')}</h2><p>${pub('发现违法、钓鱼、仿冒、恶意跳转或垃圾内容时，请联系管理员并提供域名与问题说明；不要发送密码或密钥。','Report illegal, phishing, impersonation, malicious redirects, or spam with the domain and issue details. Never send passwords or secrets.')}</p><a href="#/terms">${pub('查看使用规范','View Usage Rules')} →</a></article></section>
  `, 'public-contact');
}
...
// v113: homepage customization, strict case-sensitive image captcha, and FLORE-owned public layout.
Object.assign(I18N_EN, {
