...
let domainManagementView = { root: '', page: 1 };
...
async function renderDomains() {
  ...
  try {
    ...
    if (!roots.length) domainManagementView = { root:'', page:1 };
    if (!domainManagementView.root || !roots.includes(domainManagementView.root)) domainManagementView.root = roots[0] || '';
    const currentGroup = groups.find(([root]) => root === domainManagementView.root)?.[1] || [];
    ...
    const pagination = groups.length ? `
      <div class="domain-pagination-toolbar">
        ...
        <div class="domain-page-summary"><strong>${lang()==='en' ? `Page ${domainManagementView.page} / ${pageCount}` : `第 ${domainManagementView.page} / ${pageCount} 页`}</strong><span>${lang()==='en' ? `${currentGroup.length} domains under this root` : `该根域名共 ${currentGroup.length} 个域名`}</span></div>
